...
Result_path=/root/F13/Reboot_0905  # Path to save test log, please modify yourself
Reboot_time=43200         # Time for your reboot or powercycle test (sec)
scsi_num=1                # Type "lsscsi | wc -l" to chek your scsi drive number
GPU_num=32                # Type "lspci | grep -i xilinx | wc -l" to chek your FPGA detected amount
xb_ex=16                  # Type "xbutil examine | grep -i Yes | wc -l"
...
do
	sleep 10
	xbuex=$( xbutil examine | grep -i Yes | wc -l )
	if [ "$xb_ex" -ne "$xbuex" ];then
		echo "Waitting for driver ready"
		continue
	else
...
if [ $x -eq $scsi_num ];then
        if [ $w -eq $GPU_num ];then
			...
			speed_numa_check_all.sh 8 > $Result_path/speed_test_fail.txt
			systemctl disable Power_cycle.service
			systemctl stop Power_cycle.service
                        exit 0
                fi
