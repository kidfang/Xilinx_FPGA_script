...
do
	...
	taskset -c $c xbtest -j $path/dma_"$ty".json -d $GPU > $path/FPGA"$GPU"_dma_"$ty".txt 

#	echo -e "FPGA"$GPU"_DMA_onebyone_test  Duration:$du, Type:$ty, BS:$bs  \n"| tee -a $path/DMA_"$ty"_Summary.txt
	echo -e "FPGA"$GPU"_DMA_onebyone_test "| tee -a $path/DMA_"$ty"_Summary.txt
	echo -e "NUMA: $n, Run with CPU core: $c \n" | tee -a $path/DMA_"$ty"_Summary.txt
	cat $path/FPGA"$GPU"_dma_"$ty"* | grep -A 3 "Host <- PCIe <- FPGA" | tee -a $path/DMA_"$ty"_Summary.txt
	echo -e " \n" | tee -a $path/DMA_"$ty"_Summary.txt
	cat $path/FPGA"$GPU"_dma_"$ty"* | grep -i RESULT | tail -n 1 | tee -a $path/DMA_"$ty"_Summary.txt
	echo -e "\n-------------------------\n" | tee -a $path/DMA_"$ty"_Summary.txt
